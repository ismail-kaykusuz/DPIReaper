# DPIReaper proxy (SpoofDPI 1.2.1) - PowerShell ile derleme ve kopyalama
...
$OutExe = Join-Path $Root "spoofdpi\dpireaper-proxy.exe"
...
Write-Host "SpoofDPI (dpireaper-proxy) derleniyor..." -ForegroundColor Cyan
...
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "dpireaper-proxy.exe") -Force
Copy-Item -Path $OutExe -Destination (Join-Path $BinariesDir "dpireaper-proxy-x86_64-pc-windows-msvc.exe") -Force
Write-Host "Kopyalandi: src-tauri\binaries\dpireaper-proxy.exe (ve -x86_64-pc-windows-msvc.exe)" -ForegroundColor Green
