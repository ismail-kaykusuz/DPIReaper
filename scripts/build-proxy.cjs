#!/usr/bin/env node
/**
 * Build SpoofDPI 1.2.1 (dpireaper-proxy) and copy to spoofdpi/ and src-tauri/binaries/.
 * Requires Go in PATH.
 */
...
const outExe = path.join(root, 'spoofdpi', 'dpireaper-proxy.exe');
...
console.log('Building SpoofDPI (dpireaper-proxy) with release flags...');
const go = spawnSync(
  'go',
  ['build', '-trimpath', '-ldflags=-s -w', '-o', outExe, './cmd/spoofdpi'],
  {
    cwd: spoofDpiDir,
    stdio: 'inherit',
    shell: false,
